...
impl<T: Tray> Service<T> {
    ...
    async fn update_properties(&mut self, conn: &Connection) -> zbus::Result<()> {
        ...
        if self.label_changed() {
            // Ayatana 内联标签：发 XAyatanaNewLabel(label, guide) 信号，并把 XAyatanaLabel 放进 PropertiesChanged
            let lbl = self.get_label();
            StatusNotifierItem::<T>::x_ayatana_new_label(sni_obj.signal_emitter(), &lbl, "").await?;
            sni_changed.insert("XAyatanaLabel", lbl.into());
        }
